...
int main() {
    ...
    while (getline(cin, input) && input.length() > 0) {
        ...
        for (int i = 0; i < input.length(); i++) {
            if (input[i] == 'S') {
                startingSpot = make_pair(lines, i);
                terrain[lines].push_back(Step(i, lines, 'a'));
            } else if (input[i] == 'E') {
    ...
    queue<Step> toCheck;
    terrain[startingSpot.first][startingSpot.second].minStepsToReach = 0;
    toCheck.push(terrain[startingSpot.first][startingSpot.second]);
